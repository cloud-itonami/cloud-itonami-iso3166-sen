(ns marketentry.store
  "SSoT for the SEN market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems SN" :portal "ARMP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-sn-entity? true :has-sn-entity? true
             :requires-rccm? true :rccm-verified? true
             :drafted? false :submitted? false
             :jurisdiction "SEN" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "ARMP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-sn-entity? true :has-sn-entity? true
             :requires-rccm? true :rccm-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "ARMP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "SEN" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "ARMP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-sn-entity? true :has-sn-entity? false
             :requires-rccm? true :rccm-verified? true
             :drafted? false :submitted? false
             :jurisdiction "SEN" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "ARMP e-procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-sn-entity? true :has-sn-entity? true
             :requires-rccm? true :rccm-verified? false
             :drafted? false :submitted? false
             :jurisdiction "SEN" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "ARMP e-procurement"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-sn-entity? has-sn-entity?
                               requires-rccm? rccm-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-sn-entity?)  (assoc :engagement/requires-sn-entity? requires-sn-entity?)
    (some? has-sn-entity?)       (assoc :engagement/has-sn-entity? has-sn-entity?)
    (some? requires-rccm?)    (assoc :engagement/requires-rccm? requires-rccm?)
    (some? rccm-verified?)    (assoc :engagement/rccm-verified? rccm-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-sn-entity? :engagement/has-sn-entity?
   :engagement/requires-rccm? :engagement/rccm-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-sn-entity? (boolean (:engagement/requires-sn-entity? m))
     :has-sn-entity? (boolean (:engagement/has-sn-entity? m))
     :requires-rccm? (boolean (:engagement/requires-rccm? m))
     :rccm-verified? (boolean (:engagement/rccm-verified? m))
