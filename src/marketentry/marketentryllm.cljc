(ns marketentry.marketentryllm
  ...
  the SEN public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-sn-entity?=" (:has-sn-entity? e)
                        " rccm-verified?=" (:rccm-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-sn-entity? e))
                              (:has-sn-entity? e))
                          (or (not (:requires-rccm? e))
                              (:rccm-verified? e)))
                   0.9 0.3)}))
