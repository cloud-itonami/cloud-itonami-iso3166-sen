(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Senegal-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Senegal-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-sn-entity?
                                       true` (near-universal for Senegal
                                       ...
                                       Grounded in Senegalese legal entity
                                       (RCCM) typically required for public
                                       awards (contracting authorities / ARMP).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(RCCM/統一資格/ARMP e-procurement登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- sn-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-sn-entity? true`, INDEPENDENTLY verify
  `:has-sn-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-sn-entity?` ground truth (most Senegal public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-sn-entity? e))
                 (not (true? (:has-sn-entity? e))))
        [{:rule :sn-entity-missing
          :detail (str subject " はセネガル居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- rccm-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-rccm? true`, INDEPENDENTLY check
  `:rccm-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-rccm? e))
                 (not (true? (:rccm-verified? e))))
        [{:rule :rccm-unverified
          :detail (str subject " はRCCM確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (sn-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (rccm-unverified-violations request st)
