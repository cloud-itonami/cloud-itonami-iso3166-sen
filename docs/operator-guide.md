...
2. Register the client's intake: business type, target public function,
   prior filing history in Senegal if any.
3. Run the advisor in read-only mode against the Portail des Marchés
   Publics (marchespublics.sn).
4. Compare the checklist against the client's current documentation
   (RCCM entry via APIX's BCE).
...
- named referral relationship with Senegalese-licensed counsel or a
...
a working referral relationship with Senegalese-licensed counsel or a
registered agent for whatever licensed representation the law of
Senegal requires for actual public-procurement filings.
